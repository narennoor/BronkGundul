...
| `/report week` / `/report month` / `/report <YYYY-Www\|YYYY-MM>` | `index.js#telegramHandler` → `ensurePeriodSealed` + `formatFinancialReport` | Laporan keuangan dari seal ledger — 0 Helius. Seals a closed-but-unsealed period on demand; never reseals. `ytd`/`year` → fase 3. |

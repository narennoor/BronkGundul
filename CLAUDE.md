...
| `pnl-report.js` | ~380 | The `/pnl` report (Telegram + `scripts/pnl-report.mjs`): app bookkeeping (`lessons.json`) bridged to real on-chain SOL via the full Helius history, with gas and LLM cost. **Reporting cutoff** (`pnlReportSinceIso`, default null = whole history): the wallet is older than the agent, so pre-agent funding/swap/test txs used to pollute gas, deposits and ROI. `applyCutoff()` collapses everything before the cutoff into ONE opening-balance number (derived from the pre-cutoff flows) and drops it from every other total; ROI is then measured against `base_capital = opening balance + net deposits since`. The FULL history is still paged — the `consistent` check (`flowSum === balance`) is what proves Helius returned everything, which is what makes the derived opening balance exact. Open positions are matched against the full tx list (locked capital is locked whenever deployed) and a position deployed BEFORE the cutoff is flagged `pre_cutoff` + warned, because its outflow already sits inside the opening balance. `resolveReportCutoff` demands a leading `YYYY-MM-DD`: V8 reads `Date.parse("21 juli")` as 2001-07-21, which would silently cut off nothing. OpenRouter only reports LIFETIME key usage, so `pnlReportLlmUsdBaseline` is the reading taken at the cutoff. |

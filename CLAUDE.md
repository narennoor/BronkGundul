...
| `report` | `snapshotEnabled`, `snapshotCronUtc`, `weeklyEnabled`, `monthlyEnabled`, `yearlyEnabled` (seal+kirim laporan periode — primary only), `ytdInMonthly` (strip YTD berjalan di laporan bulanan), `ledgerRole` (primary\|contributor — contributor default so a new daemon can never double-send group reports; CopetGundul sets primary), `ledgerDir`, `registryPath`, `walkOverlapMin`, `driftToleranceSol`, `positionValuation` ("cost" — position capital is principal+rent, never market value). Flat user keys are `report*`-prefixed and registered in `CONFIG_MAP` + the `booleanKeys`/`stringKeys` sets in executor.js; `reportSnapshotEnabled`/`reportSnapshotCronUtc` changes restart cron | true, "5 0 * * *", contributor, ~/.meridian/ledger, ~/.meridian/ledger-registry.json, 30, 0.001, cost |
...
| `/report week` / `month` / `year` / `ytd` / `<YYYY-Www\|YYYY-MM\|YYYY>` | `index.js#telegramHandler` → `ensurePeriodSealed` / `buildYtdReport` + `formatFinancialReport` | Laporan keuangan dari ledger — 0 Helius. Seals a closed-but-unsealed period on demand (year seals its months first); never reseals. `ytd` is computed on demand, never sealed. |

...
| Laporan mingguan | `20 0 * * 1` (UTC) | `runPeriodReport("week")` — seal the just-closed ISO week + send via `sendHTML`. **`ledgerRole: "primary"` only** (a contributor sending too would double every report); gated by `reportWeeklyEnabled`; deferred until the closing boundary snapshot exists |
| Laporan bulanan | `30 0 1 * *` (UTC) | `runPeriodReport("month")` — same, for the just-closed month. `reportMonthlyEnabled` |
...
| `/report week` / `month` / `year` / `ytd` / `<YYYY-Www\|YYYY-MM\|YYYY>` | `index.js#telegramHandler` → `ensurePeriodSealed` / `buildYtdReport` + `formatFinancialReport` | Laporan keuangan dari ledger — 0 Helius. Seals a closed-but-unsealed period on demand (year seals its months first); never reseals. `ytd` is computed on demand, never sealed. Setiap laporan diikuti lampiran CSV §09 (`sendReportCsvs` → `sendDocument` multipart, FormData/Blob global Node 22) selama `reportCsvEnabled`. |
